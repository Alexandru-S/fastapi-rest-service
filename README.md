...
This repository is a reference project that connects a FastAPI backend and a Next.js frontend to a Postgres database. The application pulls data from the Rick and Morty API, stores it in Postgres, and exposes it through a FastAPI service that the frontend consumes.
...
- All data is persisted in Postgres.
- Data flows through the FastAPI service, where we plan to add caching and other enhancements to improve performance.
